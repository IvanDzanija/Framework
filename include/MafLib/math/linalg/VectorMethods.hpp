...
namespace maf::math {
// Inplace fill
template <Numeric T> void Vector<T>::fill(T value) noexcept {
    std::fill(_data.begin(), _data.end(), value);
}

// L2 Norm
template <Numeric T> [[nodiscard]] T Vector<T>::norm() const noexcept {
    T sum = 0.0;
...
template <Numeric T> void Vector<T>::normalize() {
    T norm = this->norm();
    if (is_close(norm, 0)) {
...
// Inplace transpose
template <Numeric T> void Vector<T>::transpose() noexcept {
